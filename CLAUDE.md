...
The frontend resolves the backend base URL at runtime in `frontend/lib/api.ts` (`resolveApiBaseUrl()`): when the page host matches `<name>-3000.<domain>` (the GitHub Codespaces forwarded-port pattern) it derives the sibling `<name>-8000.<domain>` automatically, so live verification in a browser-based Codespace needs no `NEXT_PUBLIC_API_BASE_URL` change — only that the forwarded `8000` port is set to **Public** in the Ports panel. It falls back to `NEXT_PUBLIC_API_BASE_URL` (docker-compose pins it to `http://localhost:8000`) then `localhost:8000` for local/desktop port-forwarding. `NEXT_PUBLIC_*` is inlined by `next dev`, so changing it would otherwise require recreating the frontend container — not possible from inside the git/docker-less backend dev container.
...
Known limitations observed in live runs (deferred, not blocking): ingestion runs as a FastAPI `BackgroundTask` in the server process, so the CPU-bound OCR/embedding work saturates the worker and makes concurrent `/api/...` requests (incl. `/ingest/status`) slow or time out — move to a separate worker if this matters. A long unattended run also won't survive the Codespace idle auto-stop (~30 min). The clause splitter occasionally mis-detects page-header dates (e.g. "09 February 2026") as clause numbers — harmless to retrieval but worth tightening.
